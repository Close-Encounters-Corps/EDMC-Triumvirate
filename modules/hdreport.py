...
class hyperdictionDetector():
    ...
    def setup(cls,parent,gridrow):
        ...
        cls.banner=tk.Label(cls.container,text="ПЕРЕХВАТ В ВИТЧ СПЕЙСЕ",fg="red")
        cls.banner.grid(row=0, column=0, columnspan=1, sticky="NSEW")
        cls.banner.config(font=("Arial Black", 22))
        cls.instructions=tk.Label(cls.container,text="Пожалуйста, выйдите в меню для подтверждения",fg="red")
